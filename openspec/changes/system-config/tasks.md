...
- [ ] 1.1 Create `src/OpenDockify.SystemConfig`; add `Models/Setting.cs`
  (Id, Key unique, ValueJson), `Configuration/SettingConfiguration.cs`
- [ ] 1.2 Add `SettingKeys` — allowlist with key names, value types, defaults,
  env mappings (`AI_ENDPOINT`, `AI_API_KEY`, `AI_MODEL`, `LPR_ONE_YEAR_RATE`,
  `AI_ENABLED`), and per-key validation rules
- [ ] 1.3 Implement `SystemConfigService` — typed `GetAsync<T>`,
  `SetAsync(key, value)`, cache with invalidation on write
- [ ] 1.4 Implement `IConfigurationStore` — merge precedence:
  environment variable > DB value > default
- [ ] 1.5 Add `MaskSecret` + log redaction guard for `Ai.ApiKey` patterns
- [ ] 1.6 `SystemConfigModuleExtensions.cs`; wire into `OpenDockify.Api`

## 2. Data + Migration

- [ ] 2.1 Register `Setting` DbSet + configuration in `OpenDockify.Data`
- [ ] 2.2 Extend seeder: when DB empty, insert `Ai.Enabled=false` and default
  `Lpr.OneYearRate` (from config/env)
- [ ] 2.3 `dotnet ef migrations add AddSettings` and apply

## 3. Admin API

- [ ] 3.1 Add `GET /api/admin/settings` (RequireAdmin) — all allowlisted keys,
  secrets masked
- [ ] 3.2 Add `PUT /api/admin/settings/{key}` (RequireAdmin) — allowlist
  check (unknown → 400), type/range validation, persist, invalidate cache
- [ ] 3.3 `PUT` of `Ai.ApiKey` accepts new value but reads always masked
