...
- [x] 1.1 Add `.github/workflows/ci.yml` (ported, adapted to
  `OpenDockify.sln`):
...
- [x] 1.2 Include dormant Sonar steps gated on `SONAR_TOKEN`

## 2. Verify

- [x] 2.1 Run the exact CI command sequence locally (restore → audit → format
  → build → test) and confirm each passes
- [x] 2.2 Simulate the PR coverage step:
  `python3 scripts/check_incremental_coverage.py --base origin/main
  --threshold 0.80 --reports 'TestResults/**/coverage.opencover.xml'`
  (passes — no new executable lines on clean checkout)
- [x] 2.3 Confirm the workflow file is valid YAML and references the right
