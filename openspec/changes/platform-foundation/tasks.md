...
- [ ] 1.1 `dotnet new sln -n OpenDockify` and create the projects:
...
- [ ] 1.2 Add all projects to the solution; set references:
...
- [ ] 1.3 Add `Directory.Build.props` (Nullable enable,
  `TreatWarningsAsErrors`, `AnalysisLevel=latest-recommended`,
  `LangVersion=latest`) and `.editorconfig`
- [ ] 1.4 Add `.gitignore` (bin/obj/.vs/opendockify.db)

## 2. EF Core Foundation

- [ ] 2.1 Add packages to `OpenDockify.Data`:
...
- [ ] 2.2 Implement `AppDbContext` (empty `DbSet` set at this stage) and a
  `DatabaseProvider` enum with values `Sqlite, Postgres, MySql, SqlServer`
- [ ] 2.3 Implement `DatabaseProviderRegistry` — maps provider name
...
- [ ] 2.4 Implement startup `MigrateAndSeed` — apply migrations then run the
  seeder only when the DB is empty (idempotent)
- [ ] 2.5 Wire `AddDatabaseModule(IServiceCollection, IConfiguration)` in
...
- [ ] 3.1 Add `LICENSE` (MIT) with current year + author
- [ ] 3.2 Add `README.md` — overview, quick start, SQLite-out-of-box note, a
...
- [ ] 4.1 Add multi-stage `Dockerfile`: sdk build →
...
- [ ] 4.2 Add a `/healthz` endpoint to `OpenDockify.Api`
- [ ] 4.3 Add `docker-compose.yml` (service, volume, env passthrough for
  config overrides)
- [ ] 4.4 Add `.github/workflows/ci.yml` — `dotnet build OpenDockify.sln`
  (0 warnings/0 errors) on push/PR

## 5. Verify

- [ ] 5.1 `dotnet build OpenDockify.sln` → 0 warnings / 0 errors
- [ ] 5.2 `dotnet run --project src/OpenDockify.Api` → starts, `/healthz` 200,
  SQLite file created (default provider)
- [ ] 5.3 Restart against existing SQLite file → seed is a no-op (no duplicate
  rows, no errors)
...
- [ ] 5.5 Unknown provider (`Database:Provider=oracle`) → startup fails with a
