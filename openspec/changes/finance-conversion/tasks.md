...
- [ ] 1.1 Create `src/OpenDockify.Finance`; add
...
- [ ] 1.2 Add `Services/InterestRateValidator.cs` — `Validate(ratePct, lprPct)`
  → `RateLevel` (`Ok|OverLpr|OverCap`) + Chinese warning message; cap = 4× LPR
  named constant with legal comment
- [ ] 1.3 Add adapter service `InterestRateService` that reads LPR from the
  `ISystemConfigReader` abstraction (owned by `system-config`) and delegates to
  the pure validator
- [ ] 1.4 `FinanceModuleExtensions.cs`; wire into `OpenDockify.Api`

## 2. Unit Tests

- [ ] 2.1 Create `tests/OpenDockify.Finance.Tests` (xUnit):
  - `0` → `零元整`
  - `1234` → `壹仟贰佰叁拾肆元整`
  - `1234.5` → `壹仟贰佰叁拾肆元伍角`
...
- [ ] 2.2 `InterestRateValidator` tests: rate ≤ LPR → `Ok`; LPR < rate < 4×LPR
  → `OverLpr`; rate > 4×LPR → `OverCap`; boundary == 4×LPR → `OverLpr` (not
  OverCap); changed LPR changes levels

## 3. Build & Verify

- [ ] 3.1 `dotnet build OpenDockify.sln` → 0 warnings / 0 errors
- [ ] 3.2 `dotnet test` → all finance tests pass
- [ ] 3.3 Confirm no DB/migration/endpoint changes introduced
