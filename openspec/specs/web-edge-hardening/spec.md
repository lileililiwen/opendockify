# web-edge-hardening Specification

## Purpose
Standard web-edge posture for the OpenDockify API: correlation IDs on every
response, sanitized RFC9457 errors with a stable `code` extension, deny-by-default
CORS, security headers, ForwardedHeaders behind trusted proxies, version
negotiation, OpenAPI document registry, HTTP resilience for outbound clients,
secret separation, and HTTPS-only AI endpoints.
## Requirements
