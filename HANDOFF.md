...
- `platform-ai-caching` DONE 2026-09-19, commit `3901976` on `main`, archived as `2026-09-19-platform-ai-caching`, spec `ai-caching` (+2 requirements).
  - `dotnet build OpenDockify.sln`: 0 errors, 0 warnings.
  - `dotnet test OpenDockify.sln`: 225/225 green (200 unit incl. 11 new `AiCachingTests`, 5 architecture, 20 finance).
  - `openspec validate platform-ai-caching --strict`: valid; `openspec validate --changes --strict`: no items (queue empty).
  - `flutter analyze --no-pub`: no issues; `flutter test`: 96/96 green (remaining-quota caption + scrub hint, no new widget tests).
  - HTTP smoke (`:5557`, fresh sqlite, dev): `/healthz ok`; `/readyz ok reason=ready`; login 200; `POST /api/admin/cache/invalidate {all}` → `{invalidated:[lpr,renders,ai-polish]}`; AI disabled → polish 403; AI enabled + model set with Ollama down → 200 fail-closed original + `remainingTokens:19868`; `Ai.MaxTokensPerDay=1` → polish 429 `quota_exceeded` with `resetAt`; budget reset to 20000 afterwards.
  - Unit coverage: PII scrub pre-send (ID/phone placeholders, scrubbed prompt never logged), legal-advice banner never blocks, budget 429 with zero provider calls (platform `RecordingAiProvider` + `AiTokenBudgetPolicy`), insecure-remote/unknown-provider fail-closed with zero HTTP calls, identical polish served from cache with a single provider call, SHA-256 render keys order-stable with no raw PII, LPR cached until tag invalidation, golden loan/lease renders deterministic (RMB uppercase, Chinese date).
  - Bugs found by tests/smoke/review and fixed: `CacheReadResult<decimal>.Value` is not nullable (unconstrained `T?` — gate on `Status` only); `AiTextRequest` ctor param is lowercase `timeout`; C# `\b` misses ID/phone after CJK chars (now digit lookarounds); `HybridCacheStore` needs an explicit singleton (both platform extensions use `TryAdd`); `Platform.Caching` pulls DAbstractions 9.0.3 so Finance/Templates/Rendering/AiAssist bumped `8.0.*` → `9.0.3`; `DocumentService` needs `partial` for the `LoggerMessage` host; `Ai:MaxTokensPerDay=20000` needed a per-key number-range override (all numbers were capped 0–100); `RedactSensitive` truncation 2000 → 500 (existing test updated).

## Next change

All 6 planned platform-libs changes are implemented. No active change remains.
